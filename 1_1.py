...
train_file_name = f'InternalStates/LLM_Check_Hallucination_Detection-main/data/ragtruth/Qwen/screen/ragtruth_Qwen_train_data_list.pkl'
test_file_name = f'InternalStates/LLM_Check_Hallucination_Detection-main/data/ragtruth/Qwen/screen/ragtruth_Qwen_test_data_list.pkl'
# 使用with语句确保文件正确关闭
...
print(device)
model = GNNModel(input_dim=4096, hidden_dim=512, output_dim=2).to(device)
# 设置优化器和学习率调度器
...
train()
# 保存模型参数
torch.save(model.state_dict(), f'InternalStates/LLM_Check_Hallucination_Detection-main/model/ragtruth_Qwen_gnn_model.pth')

# 加载模型参数
model.load_state_dict(torch.load(f'InternalStates/LLM_Check_Hallucination_Detection-main/model/ragtruth_Qwen_gnn_model.pth'))
