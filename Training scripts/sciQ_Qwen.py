...
def test(loader):
    ...
    return correct / len(loader.dataset),arc

for epoch in range(200):
    train()

torch.save(model.state_dict(), f'HaluGNN/model/sciQ_Qwen_cut_gnn_model1.pth')

model.load_state_dict(torch.load(f'HaluGNN/model/sciQ_Qwen_cut_gnn_model.pth'))
train_acc,_ = test(train_loader)
test_acc,tarc = test(test_loader)
