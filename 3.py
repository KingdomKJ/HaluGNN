###htruthfulQA_llama的gnn，是不是因为数据分布平衡才导致效果比较好的？
import torch
...
class GNNModel(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, k=0.5):
        ...
        self.conv2 = GraphConv(hidden_dim, 32)
        self.classifier = torch.nn.Linear(32, output_dim)
...
train_file_name = f'InternalStates/LLM_Check_Hallucination_Detection-main/data/TruthfulQA/llama/screen/TruthfulQA_llama_train_data_list.pkl'
test_file_name = f'InternalStates/LLM_Check_Hallucination_Detection-main/data/TruthfulQA/llama/screen/TruthfulQA_llama_test_data_list.pkl'
# 使用with语句确保文件正确关闭
...
train()
# 保存模型参数
torch.save(model.state_dict(), f'InternalStates/LLM_Check_Hallucination_Detection-main/model/truthfulQA_llama_gnn_model.pth')

# 加载模型参数
model.load_state_dict(torch.load(f'InternalStates/LLM_Check_Hallucination_Detection-main/model/truthfulQA_llama_gnn_model.pth'))
