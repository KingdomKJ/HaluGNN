###htruthfulQA_Qwen的gnn，是不是因为数据分布平衡才导致效果比较好的？
import torch
...
train_file_name = f'InternalStates/LLM_Check_Hallucination_Detection-main/data/TruthfulQA/Qwen/screen/TruthfulQA_Qwen_train_data_list.pkl'
test_file_name = f'InternalStates/LLM_Check_Hallucination_Detection-main/data/TruthfulQA/Qwen/screen/TruthfulQA_Qwen_test_data_list.pkl'
# 使用with语句确保文件正确关闭
...
train()
# 保存模型参数
torch.save(model.state_dict(), f'InternalStates/LLM_Check_Hallucination_Detection-main/model/truthfulQA_Qwen_gnn_model.pth')

# 加载模型参数
model.load_state_dict(torch.load(f'InternalStates/LLM_Check_Hallucination_Detection-main/model/truthfulQA_Qwen_gnn_model.pth'))
